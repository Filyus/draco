...
PointCloud::PointCloud() : num_points_(0), compression_enabled_(false) {}
...
const PointAttribute *PointCloud::GetNamedAttributeByName(
  ...
  for (size_t i = 0; i < index[static_cast<int>(type)].size(); ++i) {
    const PointAttribute *const att = attributes_[index[static_cast<int>(type)][i]].get();
    if (att->name() == name) {
      return att;
    }
  }
