...
int main() {
    ...
    size_t face_print_limit = decoded_mesh->num_faces() < 10 ? decoded_mesh->num_faces() : 10;
    for (size_t fi = 0; fi < face_print_limit; ++fi) {
        auto face = decoded_mesh->face(draco::FaceIndex(static_cast<uint32_t>(fi)));
        std::cout << "  Face " << fi << ": [" << face[0].value() 
                  << ", " << face[1].value() << ", " << face[2].value() << "]" << std::endl;
    }
