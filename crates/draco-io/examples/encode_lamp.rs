...
fn main() {
    ...
    if let Some(first_mesh) = meshes.first() {
        println!("Encoding mesh with {} faces, {} points, {} attrs", 
            first_mesh.num_faces(), first_mesh.num_points(), first_mesh.num_attributes());
        ...
        for i in 0..first_mesh.num_attributes() {
            ...
            println!("  Attr {}: {:?}, components={}, data_type={:?}", 
                     i, attr.attribute_type(), attr.num_components(), attr.data_type());
        }
        ...
        encoder.encode(&options, &mut enc_buffer).expect("Encoding failed");
        ...
        println!("Saved to {:?} ({} bytes)", output_path, enc_buffer.data().len());
    }
}
