...
fn main() {
    ...
    if let Some(mesh) = meshes.first() {
        println!("Mesh has {} faces, {} points, {} attrs", 
        ...
        if pos_id >= 0 {
            ...
            println!("Position attribute: {} unique values, {} components",
                pos_att.size(), pos_att.num_components());
            ...
            let duplicates = value_to_points.iter()
            ...
            if duplicates > 0 {
                ...
                for (i, (bytes, pts)) in value_to_points.iter()
                    .filter(|(_, pts)| pts.len() > 1)
                    .take(10)
                    .enumerate() 
                {
                    ...
                    println!("  {}: ({:.6}, {:.6}, {:.6}) used by {} points: {:?}", 
                        i, x, y, z, pts.len(), &pts[..pts.len().min(5)]);
                }
            }
        }
    }
