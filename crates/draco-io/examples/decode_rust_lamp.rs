...
fn main() {
    ...
    decoder.decode(&mut buffer, &mut mesh).expect("Failed to decode");
    
    println!("Decoded mesh: {} faces, {} points, {} attrs",
        mesh.num_faces(), mesh.num_points(), mesh.num_attributes());
    
    // Print first 5 faces
    println!("First 5 faces:");
    for f in 0..5.min(mesh.num_faces()) {
        let face = mesh.face(draco_core::geometry_indices::FaceIndex(f as u32));
        println!("  face[{}] = [{}, {}, {}]", f, face[0].0, face[1].0, face[2].0);
    }
    ...
    if pos_id >= 0 {
        ...
        for i in 0..20.min(pos_attr.size()) {
            ...
            for (c, val) in vals.iter_mut().enumerate().take(num_components.min(3)) {
                let bytes: [u8; 4] = buffer.data()[offset + c*4..offset + c*4 + 4].try_into().unwrap();
                *val = f32::from_le_bytes(bytes);
            }
            println!("  v[{}] = ({:.6}, {:.6}, {:.6})", i, vals[0], vals[1], vals[2]);
        }
        ...
        for i in 0..pos_attr.size() {
            ...
            for c in 0..num_components.min(3) {
                ...
                if val < min_vals[c] { min_vals[c] = val; }
