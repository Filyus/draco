...
/// Normalize a triangle for comparison (sort vertices, then sort components within vertex)
fn normalize_triangle(positions: &[f32], i0: u32, i1: u32, i2: u32) -> [[i64; 3]; 3] {
    // Extract vertex positions and convert to fixed-point for comparison
    let scale = 10000.0; // 0.0001 precision
    let get_vertex = |idx: u32| -> [i64; 3] {
        let base = (idx as usize) * 3;
        if base + 2 < positions.len() {
            [
                (positions[base] * scale) as i64,
                (positions[base + 1] * scale) as i64,
                (positions[base + 2] * scale) as i64,
            ]
        } else {
            [0, 0, 0]
        }
    ...
    let mut verts = [get_vertex(i0), get_vertex(i1), get_vertex(i2)];
    verts.sort();
    verts
}

/// Extract triangles from a Draco mesh as a set of normalized triangles
fn extract_triangles_from_mesh(mesh: &Mesh) -> HashSet<[[i64; 3]; 3]> {
    ...
    let num_values = pos_attr.size();
    
    // Extract all positions from buffer
    let mut positions = Vec::with_capacity(num_values * 3);
    ...
    for face_idx in 0..mesh.num_faces() {
        ...
        let i0 = pos_attr.mapped_index(face[0]).0;
        let i1 = pos_attr.mapped_index(face[1]).0;
        let i2 = pos_attr.mapped_index(face[2]).0;
        triangles.insert(normalize_triangle(&positions, i0, i1, i2));
    }

    triangles
}

/// Extract triangles from raw position/index arrays
fn extract_triangles_from_arrays(positions: &[f32], indices: &[u32]) -> HashSet<[[i64; 3]; 3]> {
    let mut triangles = HashSet::new();
    for i in (0..indices.len()).step_by(3) {
        if i + 2 < indices.len() {
            triangles.insert(normalize_triangle(positions, indices[i], indices[i + 1], indices[i + 2]));
        }
    }
    triangles
}
