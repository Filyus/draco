...
fn main() {
    let drc_path = std::env::args().nth(1).expect("Usage: verify_corner_table <drc_path>");
    ...
    decoder.decode(&mut decoder_buffer, &mut mesh).expect("Failed to decode");
    
    println!("Decoded mesh: {} vertices, {} faces", mesh.num_points(), mesh.num_faces());
