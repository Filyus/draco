...
fn test_decode_rust_encoded() {
    ...
    let path = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("../../testdata/lamp_mesh.drc");
    ...
    match decoder.decode(&mut buffer, &mut mesh) {
        Ok(()) => {
            ...
            for i in 0..mesh.num_attributes() {
                ...
                println!("  Attribute {}: type={:?}, components={}", 
                    i, att.attribute_type(), att.num_components());
            }
        }
        Err(e) => {
...
fn test_decode_cpp_encoded() {
    ...
    let path = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("../../testdata/lamp_cpp_std.drc");
    let data = fs::read(path).expect("Failed to read file");
