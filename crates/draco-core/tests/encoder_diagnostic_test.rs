        ...
        for &z_var in &z_variations {
            ...
            for &qp in &qp_values {
                for &speed in &speeds {
                    ...
                    // C++ encode
                    let cpp_drc_path = format!("temp_cpp_diag_g{}_z{}_qp{}_s{}.drc", 
                        grid_size, (z_var * 10.0) as i32, qp, speed);
                    ...
                    if let Ok(output) = output {
                        if output.status.success() {
                            ...
                            if rust_data != cpp_data {
                                let diff_info = if let Some((pos, rust_byte, cpp_byte)) = find_first_diff(&rust_data, &cpp_data) {
                                    format!("first diff at byte {}: rust=0x{:02X}, cpp=0x{:02X}", pos, rust_byte, cpp_byte)
                                } else {
                                ...
                                let rust_drc_path = format!("temp_rust_diag_g{}_z{}_qp{}_s{}.drc", 
                                    grid_size, (z_var * 10.0) as i32, qp, speed);
