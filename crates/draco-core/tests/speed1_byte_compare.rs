...
fn test_speed1_detailed_comparison() {
    ...
    println!("Mesh has {} faces, {} points", mesh.num_faces(), mesh.num_points());
    ...
    encoder.encode(&options, &mut buffer).expect("Rust encode failed");
    ...
    cpp_file.read_to_end(&mut cpp_data).expect("Failed to read cpp drc");
