...
fn test_rust_decode_cpp_encoded_grid() {
    ...
    decoder.decode(&mut decoder_buffer, &mut decoded_mesh).expect("Rust decode of C++ encoded file failed");
    
    println!("Decoded mesh: {} points, {} faces", decoded_mesh.num_points(), decoded_mesh.num_faces());
    ...
    for (i, pos) in decoded_positions.iter().enumerate() {
        println!("  Point {}: ({:.3}, {:.3}, {:.3})", i, pos[0], pos[1], pos[2]);
    }
