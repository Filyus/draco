...
fn create_mesh_with_attributes() -> Mesh {
    ...
    for y in 0..grid_size {
        for x in 0..grid_size {
            ...
            pos_attr.buffer_mut().update(&px.to_le_bytes(), Some(offset));
            pos_attr.buffer_mut().update(&py.to_le_bytes(), Some(offset + 4));
            pos_attr.buffer_mut().update(&pz.to_le_bytes(), Some(offset + 8));
        }
    }
    ...
    for y in 0..grid_size - 1 {
        for x in 0..grid_size - 1 {
            ...
            mesh.set_face(FaceIndex(face_idx), [PointIndex(p0 as u32), PointIndex(p1 as u32), PointIndex(p2 as u32)]);
            face_idx += 1;
            mesh.set_face(FaceIndex(face_idx), [PointIndex(p1 as u32), PointIndex(p3 as u32), PointIndex(p2 as u32)]);
            face_idx += 1;
        }
    }
...
fn test_quantization_bits_compatibility() {
    ...
    if obj_path.exists() { let _ = std::fs::remove_file(obj_path); }
    ...
    for qp in quantization_values {
        ...
        let output = Command::new(&encoder_path)
            .arg("-i").arg(obj_path)
            .arg("-o").arg(&cpp_drc_path)
            .arg("-method").arg("edgebreaker")
            .arg("-cl").arg(cpp_cl.to_string())
        ...
        if let Ok(output) = output {
            if output.status.success() {
                ...
                file.read_to_end(&mut cpp_data).expect("Failed to read cpp drc");
                
                let status = if rust_data == cpp_data { "MATCH" } else { "MISMATCH" };
                if rust_data != cpp_data {
                    all_passed = false;
                }
                println!("qp={:2}: Rust={:5} bytes, C++={:5} bytes -> {}", 
                         qp, rust_data.len(), cpp_data.len(), status);
                
                let _ = std::fs::remove_file(&cpp_drc_path);
            } else {
                eprintln!("C++ encoder failed for qp={}: {}", qp, String::from_utf8_lossy(&output.stderr));
                all_passed = false;
            }
        }
    ...
    assert!(all_passed, "Some quantization tests failed - see output above");
}
...
fn test_compression_levels_compatibility() {
    ...
    for cl in 0..=10 {
        ...
        let mut buffer = EncoderBuffer::new();
        let res = encoder.encode(&options, &mut buffer);
        assert!(res.is_ok(), "Rust encode failed with cl={} (speed={})", cl, speed);
        let rust_data = buffer.data().to_vec();
        ...
        if let Ok(output) = output {
            if output.status.success() {
                ...
                println!("cl={:2} (speed={:2}): Rust={:5} bytes, C++={:5} bytes -> {}", 
                         cl, speed, rust_data.len(), cpp_data.len(), status);
                
                let _ = std::fs::remove_file(&cpp_drc_path);
            } else {
                eprintln!("C++ encoder failed for cl={}: {}", cl, String::from_utf8_lossy(&output.stderr));
                all_passed = false;
            }
        }
    ...
    assert!(all_passed, "Some compression level tests failed - see output above");
}
...
fn test_quantization_edge_cases() {
    ...
    for (qp, desc) in edge_cases {
        ...
        if res.is_err() {
            println!("qp={:2} ({}): Rust encode failed (may be expected for edge case)", qp, desc);
            continue;
        }
        let rust_data = buffer.data().to_vec();
        ...
        if let Ok(output) = output {
            if output.status.success() {
                ...
                println!("qp={:2} ({:20}): Rust={:5} bytes, C++={:5} bytes -> {}", 
                         qp, desc, rust_data.len(), cpp_data.len(), status);
                
                let _ = std::fs::remove_file(&cpp_drc_path);
            } else {
                println!("qp={:2} ({}): C++ encoder failed (may be expected)", qp, desc);
            }
        }
...
fn test_speed_quantization_matrix() {
        ...
        for qp in qp_values {
            ...
            if let Ok(output) = output {
                if output.status.success() {
                    ...
                    println!("speed={:2}, qp={:2}: Rust={:5}, C++={:5} -> {}", 
                             speed, qp, rust_data.len(), cpp_data.len(), status);
    ...
    println!("\nMatrix Results: {}/{} combinations matched", match_count, total_count);
    assert_eq!(match_count, total_count, "Not all speed/quantization combinations matched");
}
