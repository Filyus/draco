...
fn encode_mesh(mesh: &Mesh, speed: i32, quantization_bits: i32) -> Result<Vec<u8>, draco_core::DracoError> {
...
fn compare_cpp_vs_rust_sphere_decode() {
    ...
    for speed in 0..=10 {
        let cpp_path = testdata.join("reference_cpp").join(format!("cpp_encoded_sphere_speed_{}.drc", speed));
        ...
        match decode_drc(&cpp_data) {
            Ok(mesh) => {
                println!("  Decoded: {} vertices, {} faces", 
                    mesh.num_points(), 
                    mesh.num_faces());
...
fn compare_cpp_vs_rust_roundtrip_sphere() {
    ...
    let cpp_path = testdata.join("reference_cpp").join("cpp_encoded_sphere_speed_5.drc");
    ...
    println!("C++ reference mesh: {} vertices, {} faces", 
        cpp_mesh.num_points(), cpp_mesh.num_faces());
...
fn binary_compare_cpp_vs_rust_encoded() {
    ...
    println!("Reference mesh: {} vertices, {} faces\n", 
        ...
        match encode_mesh(&mesh, speed, 14) {
            Ok(rust_encoded) => {
                ...
                if let Some(pos) = first_diff {
                    ...
                    println!("  Same content for first {} bytes, but different lengths", min_len);
                }
            }
            Err(e) => println!("Speed {}: Rust encode error: {:?}", speed, e),
        }
