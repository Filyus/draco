...
fn create_grid_mesh(grid_size: usize) -> Mesh {
    ...
    for y in 0..grid_size - 1 {
        for x in 0..grid_size - 1 {
            ...
            mesh.set_face(FaceIndex(face_idx as u32), [PointIndex(p0 as u32), PointIndex(p1 as u32), PointIndex(p2 as u32)]);
            face_idx += 1;
            mesh.set_face(FaceIndex(face_idx as u32), [PointIndex(p1 as u32), PointIndex(p3 as u32), PointIndex(p2 as u32)]);
            face_idx += 1;
        }
    }
...
fn benchmark_cpp_decoding(decoder_path: &Path, drc_path: &Path, iterations: u32) -> f64 {
    ...
    for _ in 0..iterations {
        ...
        if !output.status.success() {
            eprintln!("C++ decoder failed: {}", String::from_utf8_lossy(&output.stderr));
            return 0.0;
        }
...
fn benchmark_cpp_encoding(encoder_path: &Path, obj_path: &Path, speed: i32, iterations: u32) -> (f64, usize) {
    ...
    for _ in 0..iterations {
        ...
        let output = Command::new(encoder_path)
            .args([
                "-i", obj_path.to_str().unwrap(),
                "-o", drc_path.to_str().unwrap(),
                "-method", "edgebreaker",
                "-cl", &cl.to_string(),
                "-qp", "10",
            ])
        ...
        if !output.status.success() {
            eprintln!("C++ encoder failed: {}", String::from_utf8_lossy(&output.stderr));
            return (0.0, 0);
        }
        
        total_time += elapsed.as_secs_f64();
        output_size = std::fs::metadata(&drc_path).map(|m| m.len() as usize).unwrap_or(0);
    }
...
fn test_performance_comparison() {
    ...
    for &grid_size in &grid_sizes {
        ...
        println!("\nGrid {}x{} ({} points, {} faces):", grid_size, grid_size, num_points, num_faces);
        
        for &speed in &speeds {
            let (rust_time, rust_size) = benchmark_rust_encoding(&mesh, speed, iterations);
            let (cpp_time, cpp_size) = benchmark_cpp_encoding(&encoder_path, &obj_path, speed, iterations);
            ...
            println!("{:>6} {:>6} {:>10.2}ms {:>10.2}ms {:>9.2}x {:>6} {}", 
