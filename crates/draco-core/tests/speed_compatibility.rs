...
fn test_speed_compatibility() {
    let tools_path = match get_cpp_tools_path() {
        Some(p) => p,
        None => {
            eprintln!("SKIPPING comparison: C++ tools not found at {:?}", Path::new("../../build-original/src/draco/Release").canonicalize());
            return;
        }
    };
    ...
    for speed in 0..=10 {
        ...
        std::fs::write(&rust_drc_path, &rust_data).expect("Failed to write rust output for checking");
        ...
        if !decode_success {
             eprintln!("C++ decoder failed to decode Rust output at speed {}: {}", speed, String::from_utf8_lossy(&decode_output.stderr));
             println!("STDOUT: {}", String::from_utf8_lossy(&decode_output.stdout));
        }
        assert!(decode_success, "C++ decoder could not decode Rust generated drc at speed {}", speed);
        ...
        if let Ok(output) = output {
            if output.status.success() {
               ...
               println!("Speed {}: Rust Size: {}, C++ Size: {} (cl={})", speed, rust_data.len(), cpp_data.len(), cpp_cl);
               if rust_data == cpp_data {
                   println!("Speed {}: Binary MATCH", speed);
               } else {
                   println!("Speed {}: Binary MISMATCH (Rust: {}, C++: {})", speed, rust_data.len(), cpp_data.len());
               }
               // let _ = std::fs::remove_file(&cpp_drc_path); // Keep for debugging
            } else {
               if speed == 0 {
                   eprintln!("C++ encoder failed. stderr:\n{}", String::from_utf8_lossy(&output.stderr));
                   eprintln!("C++ encoder stdout:\n{}", String::from_utf8_lossy(&output.stdout));
               }
            }
        }
