    ...
    pub fn decode_values(
        ...
        for _ in 0..num_points {
            for _ in 0..num_components {
                match data_type {
                    DataType::Uint8 => {
                        let val = buffer.decode_u8().map_err(|_| {
                            DracoError::DracoError("Failed to decode u8".to_string())
                        })?;
                    ...
                    DataType::Int8 => {
                        let val = buffer.decode_u8().map_err(|_| {
                            DracoError::DracoError("Failed to decode i8".to_string())
                        })?;
                        attribute.buffer_mut().write(offset, &[val]);
                        offset += 1;
                    }
                    DataType::Uint16 => {
                        let val = buffer.decode_u16().map_err(|_| {
                            DracoError::DracoError("Failed to decode u16".to_string())
                        })?;
                    ...
                    DataType::Int16 => {
                        let val = buffer.decode_u16().map_err(|_| {
                            DracoError::DracoError("Failed to decode i16".to_string())
                        })?;
                        attribute.buffer_mut().write(offset, &val.to_le_bytes());
                        offset += 2;
                    }
                    DataType::Uint32 => {
                        let val = buffer.decode_u32().map_err(|_| {
                            DracoError::DracoError("Failed to decode u32".to_string())
                        })?;
                        attribute.buffer_mut().write(offset, &val.to_le_bytes());
                        offset += 4;
                    }
                    DataType::Int32 => {
                        let val = buffer.decode_u32().map_err(|_| {
                            DracoError::DracoError("Failed to decode i32".to_string())
                        })?;
                        attribute.buffer_mut().write(offset, &val.to_le_bytes());
                        offset += 4;
                    }
                    DataType::Uint64 => {
                        let val = buffer.decode_u64().map_err(|_| {
                            DracoError::DracoError("Failed to decode u64".to_string())
                        })?;
                        attribute.buffer_mut().write(offset, &val.to_le_bytes());
                        offset += 8;
                    }
                    DataType::Int64 => {
                        let val = buffer.decode_u64().map_err(|_| {
                            DracoError::DracoError("Failed to decode i64".to_string())
                        })?;
                        attribute.buffer_mut().write(offset, &val.to_le_bytes());
                        offset += 8;
                    }
                    DataType::Float32 => {
                        let val = buffer.decode_f32().map_err(|_| {
                            DracoError::DracoError("Failed to decode f32".to_string())
                        })?;
                        attribute.buffer_mut().write(offset, &val.to_le_bytes());
                        offset += 4;
                    }
                    DataType::Float64 => {
                        let val = buffer.decode_f64().map_err(|_| {
                            DracoError::DracoError("Failed to decode f64".to_string())
                        })?;
                        attribute.buffer_mut().write(offset, &val.to_le_bytes());
                        offset += 8;
                    }
                    DataType::Bool => {
                        let val = buffer.decode_u8().map_err(|_| {
                            DracoError::DracoError("Failed to decode bool".to_string())
                        })?;
                        attribute.buffer_mut().write(offset, &[val]);
                        offset += 1;
                    }
