...
impl MeshEdgebreakerTraversalValenceEncoder {
    ...
    pub fn init(&mut self, corner_table: &CornerTable) {
        ...
        self.corner_to_vertex_map.resize(corner_table.num_corners(), VertexIndex(0));
    ...
    pub fn encode_symbol(&mut self, symbol: EdgebreakerSymbol, corner_table: &CornerTable, visited_faces: &[bool]) {
        self.num_symbols += 1;
        ...
        match symbol {
            EdgebreakerSymbol::Center | EdgebreakerSymbol::Split => { // TOPOLOGY_C, TOPOLOGY_S
                ...
                if symbol == EdgebreakerSymbol::Split {
                     ...
                     let last_corner_v_idx = self.corner_to_vertex_map[self.last_corner.0 as usize].0 as usize;
            ...
            EdgebreakerSymbol::Right => { // TOPOLOGY_R
                self.vertex_valences[self.corner_to_vertex_map[self.last_corner.0 as usize].0 as usize] -= 1;
                self.vertex_valences[self.corner_to_vertex_map[next.0 as usize].0 as usize] -= 1;
                self.vertex_valences[self.corner_to_vertex_map[prev.0 as usize].0 as usize] -= 2;
            },
            EdgebreakerSymbol::Left => { // TOPOLOGY_L
                self.vertex_valences[self.corner_to_vertex_map[self.last_corner.0 as usize].0 as usize] -= 1;
                self.vertex_valences[self.corner_to_vertex_map[next.0 as usize].0 as usize] -= 2;
                self.vertex_valences[self.corner_to_vertex_map[prev.0 as usize].0 as usize] -= 1;
            },
            EdgebreakerSymbol::End => { // TOPOLOGY_E
                self.vertex_valences[self.corner_to_vertex_map[self.last_corner.0 as usize].0 as usize] -= 2;
                self.vertex_valences[self.corner_to_vertex_map[next.0 as usize].0 as usize] -= 2;
                self.vertex_valences[self.corner_to_vertex_map[prev.0 as usize].0 as usize] -= 2;
            },
             _ => {} // Hole?
        ...
        for symbols in &self.context_symbols {
             out_buffer.encode_varint(symbols.len() as u64);
                if !symbols.is_empty() {
                    ...
                    if !crate::symbol_encoding::encode_symbols(
                        symbols,
                        1,
                        &options,
                        out_buffer,
