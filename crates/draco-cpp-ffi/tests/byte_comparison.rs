...
fn test_byte_comparison_speed_10_simple() {
    ...
    if let Some(diff_pos) = find_first_difference(&rust_bytes, &cpp_bytes) {
        ...
        println!("Rust:");
        for (i, chunk) in rust_bytes.iter().take(100).collect::<Vec<_>>().chunks(16).enumerate() {
            print!("{:04x}: ", i * 16);
            for b in chunk {
                print!("{:02x} ", b);
            }
        ...
        println!("\nC++:");
        for (i, chunk) in cpp_bytes.iter().take(100).collect::<Vec<_>>().chunks(16).enumerate() {
            print!("{:04x}: ", i * 16);
            for b in chunk {
                print!("{:02x} ", b);
            }
...
fn test_byte_comparison_speed_10_grid() {
    ...
    if let Some(diff_pos) = find_first_difference(&rust_bytes, &cpp_bytes) {
        ...
        println!("\nRust header: {:?}", &rust_bytes[0..11.min(rust_bytes.len())]);
...
fn test_byte_comparison_all_speeds() {
    ...
    for speed in speeds {
        ...
        println!("Speed {:2}: Rust={:5} C++={:5}  {}", 
                 speed, rust_bytes.len(), cpp_bytes.len(), status);
    }
}
