...
fn test_ffi_performance_comparison() {
    ...
    for &grid_size in &grid_sizes {
        ...
        for &speed in &speeds {
            ...
            let ratio = if cpp_time > 0.0 { rust_ms / cpp_time } else { 0.0 };
...
fn test_encoding_correctness() {
    ...
    for &grid_size in &grid_sizes {
        ...
        for &speed in &speeds {
            ...
            let status = if rust_size == cpp_size { "PASS" } else { "FAIL"; all_pass = false; "FAIL" };
            println!("Grid {}x{}, speed {}: Rust={} bytes, C++={} bytes [{}]",
