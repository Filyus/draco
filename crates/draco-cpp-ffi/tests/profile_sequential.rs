...
fn profile_sequential_encoding() {
    ...
    for grid_size in [50, 100] {
        ...
        println!("Grid {}x{}: {} points, {} faces", grid_size, grid_size, num_points, num_faces);
        ...
        println!("  Rust avg: {:.2}ms  min: {:.2}ms  max: {:.2}ms", avg, min, max);
...
fn profile_detailed_breakdown() {
    ...
    println!("  Mesh clone:          {:7.1} µs ({:5.1}%)", mesh_clone_us, mesh_clone_us / total_us * 100.0);
    println!("  Options setup:       {:7.1} µs ({:5.1}%)", options_us, options_us / total_us * 100.0);
    println!("  Encoder setup:       {:7.1} µs ({:5.1}%)", encoder_setup_us - mesh_clone_us, (encoder_setup_us - mesh_clone_us) / total_us * 100.0);
    println!("  Encoding (core):     {:7.1} µs ({:5.1}%)", encoding_core_us, encoding_core_us / total_us * 100.0);
    println!("  ─────────────────────────────");
    ...
    if cpp_time >= 0 {
        ...
        println!("  {:7.1} µs (ratio: {:.2}x)", hypothetical, hypothetical / cpp_us);
    }
}
...
fn profile_encoding_stages() {
    ...
    for _ in 0..iterations {
        ...
        for i in 0..num_points {
            ...
            let z = u32::from_le_bytes([data[offset+8], data[offset+9], data[offset+10], data[offset+11]]);
    ...
    for _ in 0..iterations {
        ...
        let max_correction_adj = if (max_dif & 1) == 0 { max_correction - 1 } else { max_correction };
        ...
        while i >= 3 {
            ...
            if corr_x < min_correction { corr_x = corr_x.wrapping_add(max_dif); }
            else if corr_x > max_correction_adj { corr_x = corr_x.wrapping_sub(max_dif); }
            if corr_y < min_correction { corr_y = corr_y.wrapping_add(max_dif); }
            else if corr_y > max_correction_adj { corr_y = corr_y.wrapping_sub(max_dif); }
            if corr_z < min_correction { corr_z = corr_z.wrapping_add(max_dif); }
            else if corr_z > max_correction_adj { corr_z = corr_z.wrapping_sub(max_dif); }
    ...
    let symbols: Vec<u32> = values.iter().map(|&val| {
        if val < 0 {
            ((-val as u32) << 1) - 1
        } else {
            (val as u32) << 1
    ...
    for _ in 0..iterations {
        ...
        let options = SymbolEncodingOptions { compression_level: 7 };
    ...
    // Print results
    let total_staged = quant_compute_us + quant_apply_us + gather_us + delta_wrap_us + zigzag_us + symbol_encode_us;
    
    println!("Stage breakdown (avg over {} iterations):", iterations);
    println!("  1. Quantization compute:  {:7.1} µs ({:5.1}%)", quant_compute_us, quant_compute_us / total_staged * 100.0);
    println!("  2. Quantization apply:    {:7.1} µs ({:5.1}%)", quant_apply_us, quant_apply_us / total_staged * 100.0);
    println!("  3. Value gathering:       {:7.1} µs ({:5.1}%)", gather_us, gather_us / total_staged * 100.0);
    println!("  4. Delta + wrap:          {:7.1} µs ({:5.1}%)", delta_wrap_us, delta_wrap_us / total_staged * 100.0);
    println!("  5. Zigzag encoding:       {:7.1} µs ({:5.1}%)", zigzag_us, zigzag_us / total_staged * 100.0);
    println!("  6. Symbol encoding:       {:7.1} µs ({:5.1}%)", symbol_encode_us, symbol_encode_us / total_staged * 100.0);
    println!("  ─────────────────────────────────────");
...
fn profile_symbol_encoding_details() {
    ...
    let symbols: Vec<u32> = (0..num_values).map(|i| {
    ...
    let _bit_lengths: Vec<u32> = symbols.chunks(num_components).map(|chunk| {
        let max_comp = *chunk.iter().max().unwrap();
        if max_comp > 0 { 32 - max_comp.leading_zeros() } else { 1 }
    }).collect();
    ...
    let mut prob_table: Vec<RAnsSymbol> = frequencies.iter().map(|&freq| {
        let prob = freq as f64 / total_freq_d;
        let mut rans_prob = (prob * rans_precision_d + 0.5) as u32;
        if rans_prob == 0 && freq > 0 {
            rans_prob = 1;
        }
        RAnsSymbol { prob: rans_prob, cum_prob: 0 }
    }).collect();
    ...
    println!("Symbol encoding breakdown (avg over {} iterations, {} symbols):", iterations, symbols.len());
    println!("  A. Compute bit lengths:   {:7.1} µs ({:5.1}%)", bit_lengths_us, bit_lengths_us / total_measured * 100.0);
    println!("  B. Compute frequencies:   {:7.1} µs ({:5.1}%)", freq_count_us, freq_count_us / total_measured * 100.0);
    println!("  C. rANS table creation:   {:7.1} µs ({:5.1}%)", table_create_us, table_create_us / total_measured * 100.0);
    println!("  D. rANS encoding loop:    {:7.1} µs ({:5.1}%)", rans_loop_us, rans_loop_us / total_measured * 100.0);
    println!("  ─────────────────────────────────────");
    println!("  Isolated total:           {:7.1} µs", total_measured);
    println!("  Full encode_symbols():    {:7.1} µs", full_encode_us);
    println!();
    println!("Overhead in encode_symbols: {:.1} µs ({:.1}%)", 
...
fn profile_rans_loop_micro() {
    ...
    let symbols: Vec<u32> = (0..num_symbols).map(|i| {
        let mut h = DefaultHasher::new();
        i.hash(&mut h);
        (h.finish() as u32) % (max_value + 1)
    }).collect();
    ...
    for _ in 0..iterations {
        ...
        for &symbol in symbols.iter().rev() {
            ...
            while state >= renorm_bound {
                unsafe { *buf.get_unchecked_mut(buf_offset) = (state & 0xFF) as u8; }
                buf_offset += 1;
                state >>= 8;
            }
    ...
    println!("rANS loop approaches ({} symbols, {} iterations):", num_symbols, iterations);
    ...
    println!();
    println!("Speedup from Vec::push to unchecked: {:.2}x", vec_push_us / unchecked_us);
}
...
fn profile_full_encode_breakdown() {
    ...
    let face_data: Vec<[VertexIndex; 3]> = (0..num_faces).map(|i| {
        let f = mesh.face(FaceIndex(i as u32));
        [VertexIndex(f[0].0), VertexIndex(f[1].0), VertexIndex(f[2].0)]
    }).collect();
    ...
    println!("Full encode breakdown ({}x{} mesh, {} iterations):", grid_size, grid_size, iterations);
    ...
    println!();
    println!("CornerTable as % of full: {:.1}%", corner_table_us / full_encode_us * 100.0);
    println!("Mesh clone as % of full: {:.1}%", mesh_clone_us / full_encode_us * 100.0);
    ...
    let start = Instant::now();
    for _ in 0..iterations {
        let mut corner_to_vertex = vec![draco_core::geometry_indices::INVALID_VERTEX_INDEX; num_faces * 3];
    ...
    println!("  B. Rest of init:          {:7.1} µs ({:.1}%)", 
...
fn profile_rust_vs_cpp_breakdown() {
    ...
    println!("Grid {}x{}: {} points, {} faces ({} iterations)\n", 
    ...
    for speed in [0, 5, 10] {
        ...
        // C++ Profile
        let cpp_profile = draco_cpp_ffi::profile_cpp_encode(
            &positions,
            &faces,
            speed,
            speed,
        ...
        println!("  Mesh clone:       {:7.1} µs ({:.1}%)", 
                 rust_mesh_setup, rust_mesh_setup / rust_total * 100.0);
        println!("  Encoder setup:    {:7.1} µs ({:.1}%)", 
                 rust_encoder_setup, rust_encoder_setup / rust_total * 100.0);
        println!("  Actual encode:    {:7.1} µs ({:.1}%)", 
                 rust_encode, rust_encode / rust_total * 100.0);
        println!("  TOTAL:            {:7.1} µs\n", rust_total);
        
        // Comparison
        println!("Comparison (encode only):");
        println!("  Rust encode:  {:7.1} µs", rust_encode);
        println!("  C++ encode:   {:7.1} µs", cpp_profile.encode_time_us as f64);
        println!("  Ratio:        {:.2}x {}", 
                 rust_encode / cpp_profile.encode_time_us as f64,
                 if rust_encode < cpp_profile.encode_time_us as f64 { "(Rust faster)" } else { "(C++ faster)" });
        
        println!("\nComparison (total with mesh setup):");
        println!("  Rust total:   {:7.1} µs", rust_total);
        println!("  C++ total:    {:7.1} µs", cpp_profile.total_time_us as f64);
        println!("  Ratio:        {:.2}x {}", 
                 rust_total / cpp_profile.total_time_us as f64,
                 if rust_total < cpp_profile.total_time_us as f64 { "(Rust faster)" } else { "(C++ faster)" });
        
        println!("\nOutput sizes: Rust={} C++={} {}\n", 
                 rust_output_size, cpp_profile.output_size,
                 if rust_output_size == cpp_profile.output_size { "✓" } else { "✗ MISMATCH" });
        println!("{}\n", "-".repeat(50));
    }
}
...
fn profile_decode_rust_vs_cpp() {
    ...
    for speed in [0, 5, 10] {
        ...
        println!("  Ratio:      {:.2}x {}", 
                 ratio,
                 if ratio < 1.0 { "(Rust faster)" } else { "(C++ faster)" });
        ...
        println!("  Points:     {} vs {} {}", 
                 rust_num_points, cpp_result.num_points, 
                 if points_match { "✓" } else { "✗" });
        println!("  Faces:      {} vs {} {}", 
                 rust_num_faces, cpp_result.num_faces, 
                 if faces_match { "✓" } else { "✗" });
...
fn profile_decode_sequential_breakdown() {
    ...
    for _ in 0..iterations {
        ...
        // Decode
        let start = Instant::now();
        decoder.decode(&mut decoder_buffer, &mut out_mesh).expect("Decode failed");
    ...
    println!("\nRatio: {:.2}x {}", ratio, if ratio < 1.0 { "(Rust faster)" } else { "(C++ faster)" });
}
