...
fn bench_cpp_decode(data: &[u8], iters: u32) -> Option<(f64, usize, usize)> {
    ...
    Some((r.decode_time_us as f64, r.num_points as usize, r.num_faces as usize))
}
...
fn decode_real_files_comparison() {
    ...
    let reference_files: Vec<FileCase> = (0..=10).map(|s| FileCase {
        label: Box::leak(format!("sphere speed {}", s).into_boxed_str()),
        path: vec!["reference_cpp", Box::leak(format!("cpp_encoded_sphere_speed_{}.drc", s).into_boxed_str())],
        iters: 200,
    }).chain((0..=10).map(|s| FileCase {
        label: Box::leak(format!("cube   speed {}", s).into_boxed_str()),
        path: vec!["reference_cpp", Box::leak(format!("cpp_encoded_cube_speed_{}.drc", s).into_boxed_str())],
        iters: 200,
    })).collect();

    let real_files: &[FileCase] = &[
        FileCase { label: "bunny (cpp)",          path: vec!["bunny_cpp.drc"],          iters: 50  },
        FileCase { label: "bunny (cpp standard)", path: vec!["bunny_cpp_standard.drc"], iters: 50  },
        FileCase { label: "cube_att",             path: vec!["cube_att.drc"],           iters: 200 },
        FileCase { label: "annulus (edgbrk)",     path: vec!["annulus_eb.drc"],         iters: 200 },
        FileCase { label: "car",                  path: vec!["car.drc"],                iters: 50  },
    ];
    ...
    for case in &reference_files {
        ...
        for seg in &case.path { p = p.join(seg); }
        let data = match std::fs::read(&p) {
            Ok(d) => d,
            Err(_) => {
                println!("│ {:<20} │  n/a   │   MISSING │   MISSING │    -    │    -    │", case.label);
                continue;
            }
        };
        ...
        match (rust, cpp) {
            (Some((r_us, r_pts, r_faces)), Some((c_us, c_pts, c_faces))) => {
                ...
                let ok = if r_pts == c_pts && r_faces == c_faces { "✓" } else { "✗" };
                println!("│ {:<20} │{:>7} │ {:>9.1} │ {:>9.1} │ {:>6.2}x{} │ {:>7} │",
                    case.label, data.len(), r_us, c_us, ratio,
                    ok, winner);
            }
            (None, _) => println!("│ {:<20} │{:>7} │ RUST FAIL │ {:>9.1} │    -    │    -    │",
                case.label, data.len(),
                cpp.map(|c| c.0).unwrap_or(0.0)),
            (_, None) => println!("│ {:<20} │{:>7} │ {:>9.1} │  FFI FAIL │    -    │    -    │",
                case.label, data.len(),
